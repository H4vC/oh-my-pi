...
- `tab` helpers; drop to raw Playwright `page` for anything uncovered:
  ...
  - `tab.waitFor(selector)` — wait until attached; returns `Locator` (headless) or `CmuxElementHandle` (cmux); both support `.click()`, `.type()`, `.fill()`.
...
- Selectors: CSS + Playwright engines `text=Sign in`, `xpath=//a`, `role=button[name="Save"]`; legacy `aria/X`, `text/X`, `xpath/X`, `pierce/X`, `p-aria/X`, `p-text/X` auto-translated. Bare `aria/Name` matches by computed accessible name (aria-label, aria-labelledby, title, visible text) via a custom engine. `role=`/`aria/role[name="…"]` match by ARIA role (incl. implicit) + accessible name in both backends.
</instruction>
